...
gulp.task('css', (done) => {
  ...
  gulp
    .src(`${paths.projectDir}/src/sass/ukti-template.scss`)
    .pipe(
      sass({
  ...
  gulp
    .src(`${sassSrc}/trade-elements.scss`)
    .pipe(
      sass({ outputStyle: (config.env === 'production') ? 'compressed' : 'nested' })
        .on('error', sass.logError))
    .pipe(autoprefixer({ browsers: ['> 1%', 'last 2 versions', 'IE 9'] }))
    .pipe(gulp.dest(outputCss))
